...
public class ParallelSearcher<M extends Move<M>, B extends Board<M, B>> extends
	...
	private static final int DIVIDE_CUTOFF = 32;
		...
		protected BestMove<M> compute() {
			if(this.size() <= this.divideCutoff) {
				...
				for(int i = l; i < r - 1; i++) {
					...
					taskList.add(new GetBestMoveTask<M, B>(null, board.copy(), 0, -1, e, depth - 1, cutoff, divideCutoff, true));
					taskList.get(i - l).fork();
					board.undoMove();
				}
				board.applyMove(moves.get(r - 1));
				taskList.add(new GetBestMoveTask<M, B>(null, board, 0, -1, e, depth - 1, cutoff, divideCutoff, true));
