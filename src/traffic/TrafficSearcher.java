...
public class TrafficSearcher<M extends Move<M>, B extends Board<M, B>> extends
    ...
    static class TrafficSubTask<M extends Move<M>, B extends Board<M, B>> extends RecursiveTask<BestMove<M>> {
    	...
    	public TrafficSubTask(Evaluator<B> e, B board, M move, int depth, List<M> moves, int l, int r, int alpha, int beta, int cutoff, int divideCutoff, boolean AlreadyHaveGoodAlphaBeta) {
    		this.e = e;
    		this.move = move;
    		this.board = board;
    		this.depth = depth;
		...
		protected BestMove<M> compute() {
			...
			if(!AlreadyHaveGoodAlphaBeta) {
		    	...
		    	for (int i = l; i < l + (int) (PERCENTAGE_SEQUENTIAL * this.size()); i++) {
		    		...
		    		int value = new TrafficSubTask<M, B>(this.e, this.board, null, this.depth - 1, null, 0, -1, -this.beta, -this.alpha, this.cutoff, this.divideCutoff, false).compute().negate().value;
		    		this.board.undoMove();
		    		if (value > alpha) {
		    			alpha = value;
		    			bestMove = move;
			...
			if(this.size() > this.divideCutoff) {
				TrafficSubTask<M, B> leftTask = new TrafficSubTask<M, B>(e, board, this.move, depth, moves, st, st + (ed - st) / 2, alpha, beta, cutoff, divideCutoff, true);
				TrafficSubTask<M, B> rightTask = new TrafficSubTask<M, B>(e, board, this.move, depth, moves, st + (ed - st) / 2, ed, alpha, beta, cutoff, divideCutoff, true);
				...
				List<TrafficSubTask<M, B>> taskList = new ArrayList<TrafficSubTask<M, B>>();
		    	for (int i = st; i < ed - 1; i++) {
		    		//board.applyMove(this.moves.get(i));
		    		taskList.add(new TrafficSubTask<M, B>(this.e, this.board, this.moves.get(i), this.depth - 1, null, 0, -1, -this.beta, -this.alpha, this.cutoff, this.divideCutoff, false));
		    	...
		    	TrafficSubTask<M, B> current = new TrafficSubTask<M, B>(this.e, this.board, this.moves.get(ed - 1), this.depth - 1, null, 0, -1, -this.beta, -this.alpha, this.cutoff, this.divideCutoff, false);
